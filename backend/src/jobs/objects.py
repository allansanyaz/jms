import xml.etree.ElementTree as ET #replace all instances based on ET
import os, socket
from utilities.io.shell import UserProcess
from lxml import objectify # replace objectify with dicts
import json
...
class DependencyCondition: #TODO: check what this class does. May not be needed for openpbs
    Success = 1
    Failed = 2
    Completed = 3
    Exit_Code = 4
...
class Dashboard:
    ...
    def GetNodes(self, process):
        nodes = []
        #out0 = process.run_command("qnodes -x")
        out = process.run_command("pbsnodes -aFjson")
        #root0 = ET.fromstring(out0)
        data = json.loads(out)
        ...
        for node in node_dict:
            ...
            num_cores = int(node_dict[node]["resources_available"]["ncpus"])
            properties = state
            busy_cores = 0;            
            job_dict = dict()
            jobs = node.find('jobs') #TODO: fix parsing issues here, if this code is active
            if jobs is not None:            
                jobs = jobs.text
...
class Queue:
    def __init__(self, QueueName=None, Type=None,
                 Enabled=None, Started=None, MaxQueable=None,
                 MaxRun=None, MaxUserQueable=None,
                 MaxUserRun=None, MaxNodes=None,
                 DefaultNodes=None, MaxCPUs=None,
                 DefaultCPUs=None, MaxMemory=None,
                 DefaultMemory=None, MaxWalltime=None,
                 DefaultWalltime=None, DefaultQueue=False):
...
class Client:
    def __init__(self, NodeName=None, State=None,
                 NumProcessors=None, Properties=None):
