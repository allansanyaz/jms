...
from jobs.JMS.CRUD import ToolPermissions
...
class CategoryDetail(APIView):
    ...
    def put(self, request, category_id):
        ...
        category_name = list(request.data)[0]
...
class ToolVersionList(APIView):
    ...
    def post(self, request, tool_id):
        ...
        """
        data = list(request.data)
        version_nums = json.loads(data[0])
...
class ToolShareDetail(APIView):
    ...
    def put(self, request, tool_id, user_name):
        ...
        """
        data = request.data
        permissions = json.loads(list(data)[0])
...
class CustomJob(APIView):
    ...
    def post(self, request):
        ...
        jms = JobManager(user=request.user)
        job = jms.CreateJob(job_name, description,
                JobTypeID=1,
                NotificationMethod=notification_method,
                NotificationURL=notification_url,
                NotificationEmail=notification_email)
        jobstage = jms.RunCustomJob(job, commands,
                        settings, files)
...
class ToolJob(APIView):
    ...
    def post(self, request, version_id):
        ...
        try:
            ...
            files = []
            for k, v in request.FILES.items(): #changed from iteritems
