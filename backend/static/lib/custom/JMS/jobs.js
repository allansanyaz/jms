...
function JobsViewModel() {
	...
	//Filters			
	/*self.JobFilter = ko.observable("");
	self.JobFilter.subscribe(function(){
	    self.FilterJob();
	});*/
	...
	self.Filter = function(job_row) {
		if(self.JobFilter().length > 0) {
			if(job_row.JobID().indexOf(self.JobFilter()) >= 0) {
			   	return true;
		    } else { 
		        var found = false;
			   	$.each(job_row.Values(), function(i, val) {
			   	    ...
			   	    index = val.indexOf(self.JobFilter());
	    ...
	    $.ajax({
	        ...
	        success: function(j) {
	            ...
	            $.each(j.JobStages, function(i, js) {
	                ...
	                console.log(js); // TODO:comment out later
